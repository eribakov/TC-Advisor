...
async function sendToBackground(text: string, source: string): Promise<ScanResult> {
  ...
  return new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { action: 'callGemini', text, source },
      (response?: unknown) => resolve((response as ScanResult | undefined) ?? { error: 'No response from background' })
    );
  });
}
...
chrome.runtime.onMessage.addListener((
  ...
  if (typeof msg === 'object' && msg !== null && 'action' in msg && (msg as { action: string }).action === 'scan') {
